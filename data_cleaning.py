...
def data_cleaning():
    ...
    for i, row in enumerate(sample):
        ...
        # 某块磁盘数据太久未收集当作新盘对待
        if i + 1 < len(sample) and date_sub(sample[i + 1][0], sample[i][0]) >= DIVIDE_DISTANCE:
            serial_list[len(serial_list) - 1].append(row)
            serial_list.append([])
            continue
        serial_list[len(serial_list) - 1].append(row)
    ...
    for disk_id, disk in enumerate(serial_list):
        ...
        while disk_id < len(serial_list) and (len(serial_list[disk_id])/(date_sub(serial_list[disk_id][len(serial_list[disk_id]) - 1][0],serial_list[disk_id][0][0]) + 1) < DROP_RATE or len(serial_list[disk_id]) <= DROP_NUM):
...
def data_serializing(time_step):
    ...
    np.save("./temp_data/sample.npy", sample)
    np.save("./temp_data/label.npy", step_label)
    print("序列化完毕!")
